...
class GoFish(object):
    ...
    def play_go_fish(self):
        ...
        while self.deck.cards_remaining()>0:
            ...
            for p in range(1,self.num_players+1):
                print("Player {p} has {s} sets:".format(p=p,s=len(self.players[p-1].sets)))
            ...
            if self.debug==True:
                print("Shh.. Here's the other player's hand(!!!):")
    ...
    def get_guess_value(self):
        ...
        while (Card.values.count(guess_value)==0):
             guess_value = input('\nWhat card would you like to guess (1-10,J,Q,K)? ')
        return guess_value
...
def main():
    debug = input("Start debug mode (y/n)? ")

    go_fish = GoFish(4, debug in ['Y','y'])
